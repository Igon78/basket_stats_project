...
def createPlayers():
    ...
    style.configure("Treeview", background="#0BBFFF", foreground="black", fieldbackground="#0BBFFF", bordercolor="black")
    style.configure("Treeview.Heading", font=("Arial", 10, "bold"), background="#0BBFFF", foreground="black", bordercolor="black")
    ...
    def showPlayerStats():  # funkcja do wyświetlenia informacji o graczu
        ...
        time.sleep(0.7)
        season_stats = playercareerstats.PlayerCareerStats(player_id=player_id, per_mode36='PerGame').get_data_frames()[0]
    ...
    searchEntry.pack()
    searchButton = Button(searchFrame, text="Get player data", background='#0BBFFF', foreground="black", command=showPlayerStats)
...
def createHeatMap(side=0.0):
    ...
    def show_shot_graph(season='2022-23'):
        ...
        if shoot_stats.empty:  # wychwycenie problemu z brakiem danych dla zawodnika w podanym sezonie
            ...
            def draw_court(ax=None, color='black', lw=2, outer_lines=False):
                ...
                if ax is None:
                    ax = plt.gca()
                    img = Image.open('../../Desktop/nba_floor.png')
    ...
    style = ttk.Style()
    style.configure("TCombobox", background="#0BBFFF", foreground="black", fieldbackground="#0BBFFF", bordercolor="black")
    ...
    searchButton = Button(searchFrame, text="Show shoot graph", background='#0BBFFF', foreground="black",
...
def createStandings():
    ...
    westLabel = Label(standingsFrame, text='West Conference', font=("Arial", 20), background='#0BBFFF', foreground="black")
    ...
    eastLabel = Label(standingsFrame, text='East Conference', font=("Arial", 20), background='#0BBFFF', foreground="black")
...
def createPredictions():
    ...
    searchButton = Button(predictionsFrame, text="Make predictions", command=predict_outcome, background='#0BBFFF', foreground="black")
...
def createSimilar():
    ...
    def showPlayerTwin():  # funkcja do wyświetlenia informacji o graczu
        ...
        all_players_data = all_players_data[['PTS', 'AST', 'REB', 'BLK', 'STL']]
        my_player = my_player[['PTS', 'AST', 'REB', 'BLK', 'STL']]
        ...
        similar_id = similar_player.name
        df3 = similar.loc[similar_id]
        similar_id = df3.PLAYER_ID
        df3 = similar.loc[similar['PLAYER_ID'] == similar_id]
        df4 = similar.loc[similar['PLAYER_ID'] == int(player_id)]
        try:
            tree.delete('row')  # usunięcie danych z tabeli dotyczących wcześniej wyszukanego zawodnika
        except Exception:
            pass
    ...
    searchEntry = AutocompleteCombobox(searchFrame, width=30, completevalues=playersArray)
    searchEntry.pack()
    searchButton = Button(searchFrame, text="Find his twin", background='#0BBFFF', foreground="black", command=showPlayerTwin)
    searchButton.place(relx=0, rely=0.45, relheight=0.5, relwidth=1)
