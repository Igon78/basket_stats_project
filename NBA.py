...
def createHeatMap():
    ...
    plotLabel = Label(heatmapFrame, bg='#ffffff')
    plotLabel.place(relx=0.65, rely=0.1, relheight=0.6, relwidth=0.55, anchor='n')
    def show_shot_graph():

        # pobranie i przygotowanie danych o rzutach zawodnika z wybranego sezonu
        player_id = pd.DataFrame(players.find_players_by_full_name(searchEntry.get())).id[0]
        shoot_stats = shotchartdetail.ShotChartDetail(player_id=player_id,season_nullable="2021-22",team_id=0,context_measure_simple='FGA',
        ...
        plt.hexbin(x=shoot_stats.LOC_X, y=shoot_stats.LOC_Y, C=shoot_stats.EVENT_TYPE, gridsize=40,  cmap='Blues')
